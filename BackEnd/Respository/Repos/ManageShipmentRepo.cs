...
public class ManageShipmentRepo : IManageShipmentRepo
{
    ...
    public async Task<bool> UpdateShipmentMap(string shipId)
    {
        var shipments = await _db.Shipments.FirstOrDefaultAsync(o => o.ShipmentId == shipId);
        var quotes = await _db.Quotes.FirstOrDefaultAsync(o => o.ShipmentId == shipId);
        shipments.Status = "Picked Up";
        quotes.Status = "Picked Up";
        //  _db.Shipments.Update(shipments);
        await _db.SaveChangesAsync();
        return true;
    }
 ...
 .FirstOrDefaultAsync();
    ...
    public async Task<List<ShipmentAfterPaymentDto>> GetShipmentsAfterPaymentRawAsync(string userId, string role)
    {
        ...
        switch (role.ToLower())
        {
            ...
            SELECT 
                ...
                s.ShipmentId AS ShipmentCode,
                s.Status AS Status,
                tr.FirstName + ' ' + tr.LastName AS Transporter,
                tr.PhoneNumber AS TransporterPhoneNumber
            FROM Quotes q
            INNER JOIN Shipments s ON s.ShipmentId = q.ShipmentId
            INNER JOIN AspNetUsers cu ON cu.Id = s.UserId
            INNER JOIN AspNetUsers tr ON tr.Id = q.TransId
            WHERE q.TransId = {0}   AND s.TransporterId= q.TransporterId
AND (s.Status = 'Completed' OR s.Status = 'Picked Up')";
                break;
            case "customer":
                sql = @"
            SELECT 
                ...
                tr.PhoneNumber AS TransporterPhoneNumber
            FROM Shipments s
            INNER JOIN Quotes q ON q.ShipmentId = s.ShipmentId
            INNER JOIN AspNetUsers cu ON cu.Id = s.UserId
            INNER JOIN AspNetUsers tr ON tr.Id = q.TransId
            WHERE s.UserId = {0}   AND s.TransporterId= q.TransporterId 
AND (s.Status = 'Completed' OR s.Status = 'Picked Up')";
                break;
            default:
                break;
        ...
        return await _db.Database.SqlQueryRaw<ShipmentAfterPaymentDto>(sql, userId)
            .ToListAsync();
    }
    ...
    public async Task<IEnumerable<ShipmentResponsForLandingeDto>> GetShipmentsLanding()
    {
        var shipments = await (
            ...
            where s.Status == "Accepted" || s.Status == "In-Transit"
