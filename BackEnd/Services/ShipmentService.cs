...
public class ShipmentService : IShipmentService
{
    ...
    public async Task<ApiResponse<List<ShipmentAfterPaymentDto>>> GetShipmentAfterPayment()
    {
        var loggedInUser = await _authService.ValidateRequest();
        var result = await _unitOfWork.ManageShipmentRepo.GetShipmentsAfterPaymentRawAsync(loggedInUser.UserId, loggedInUser.Role);

        return new ApiResponse<List<ShipmentAfterPaymentDto>>
        {
            Data = result,
            Message = "Shipments retrieved successfully",
            Status = true,
